...
export default function CountriesCard({ country }) {
    ...
    return (
        <Link to={`/the-world/country/${cca2}`} className={`${darkMode ? 'dark' : ''} hover:scale-105 transition-all ease-in duration-100 `}>
            <div className='px-6 py-3 cursor-pointer rounded-lg text-center shadow-light dark:text-white  dark:shadow-dark w-72 h-48 max-sm:w-40 max-sm:h-32'>
                ...
                <div className='text-xl font-medium mt-1 max-sm:mt-0 truncate'>{country.name.common}</div>
            </div>
        </Link>
    )
}